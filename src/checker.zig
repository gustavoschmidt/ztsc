//! Checker core: structural assignability, inference, literal widening,
//! control-flow narrowing, tsc-coded diagnostics.
//!
//! Scope & stance (documented deviations are intentional for v0.0.1):
//!
//! - **Single file, single thread.** Later phases add the module graph and N-checker
...
const Checker = struct {
    ...
    fn typeFromTypeName(c: *Checker, name_node: Node, args: []const TypeId) Error!TypeId {
        ...
        switch (c.resolveSpace(a, c.cur_scope, false)) {
            .sym => |sym| {
                ...
                if (f.import_binding and f.type_only) return types.any_type; // module graph
    ...
    fn computeTypeOfSymbol(c: *Checker, sym: SymbolId) Error!TypeId {
        ...
        if (f.import_binding) return types.any_type; // module graph
