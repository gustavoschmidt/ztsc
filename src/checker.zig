...
pub const Stats = struct {
    ...
    /// Equals `types_created`/`type_bytes` under `--no-frozen-store`;
...
/// Build the shared frozen base type store (architecture, frozen-base piece 2
...
/// `Program.merged` range for cross-file merges (the cross-file merge table, reused
/// per merge invariant 6) — and forces both its value type (`typeOfSymbol`) and
...
/// `makeIntersection`), and the display-order fix decoupled *display* from storage order:
...
const Checker = struct {
    ...
    fn init(
        ...
        /// this checker — the frozen-base builder (frozen-base piece 2) passes the
    ...
    /// Pre-expand every lib/`@types` global into this checker's store (payload
    ...
    fn interfaceConstituentObject(c: *Checker, sym: SymbolId) Error!TypeId {
        ...
        // The frame is this interface's own `interfaceGeneric`
