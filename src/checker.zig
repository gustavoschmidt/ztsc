...
pub const Checker = struct {
    ...
    overload_group_pool: std.ArrayListUnmanaged(u32) = .empty,
    ...
    /// Partial REVERSE index of `class_static_cache`: the class a materialized
    /// static-side object belongs to, for the one rule that needs the static
    /// side to name a class — the `private`/`protected` screen inside the
    /// relation (`nominal_members.zig`, which has a `.ref` to read the
    /// declaration side off on the INSTANCE side and nothing at all here).
    ///
    /// One entry per class with a static member, written where
    /// `class_static_cache` is.
    ///
    /// Object types are hash-consed, so two classes with byte-identical static
    /// tables share one id and the key cannot name either: such an entry is
    /// POISONED (`ambiguous_static_owner`) and the rule falls back to its
    /// no-declaration answer, which is "related" — the under-report ztsc had
    /// before the index existed, never a false positive.
    class_static_owner: IntMap(TypeId, SymbolId) = .empty,
    ...
    /// Classes whose `extends` clause `baseClassRef` is presently resolving.
    /// The clause is an EXPRESSION, so resolving it can run arbitrary checking
    /// and come back around: `class Test { p: any = class extends this.p {} }`
    /// asks for the anonymous class's base, which checks `this.p` under the
    /// anonymous class's own `this`, which looks `p` up through its bases —
    /// `baseClassRef` again, forever. Re-entry answers "no base", the same
    /// answer a genuine `extends` cycle gets.
    base_ref_active: IntMap(SymbolId, void) = .empty,
    ...
    /// Alias symbols that lie ON a materialization cycle: every member of the
    /// suffix `markCycle` cuts, not just the one this checker instance happened
    /// to enter first. See `markCycle` (instantiate.zig) for why the whole
    /// suffix is the only order-independent answer, and `alias_stack` for the
    /// frame stack it reads.
    ///
    /// Two consumers: the ONE-SPELLING rule in `aliasInstance` (a marked alias
    /// answers with a lazy `.ref` instead of a fresh structural
    /// materialization) and the default-substitution scoping in `fixTypeArgs`.
    alias_recursive: IntMap(SymbolId, void) = .empty,
    /// The subset of `alias_recursive` whose cycle is length ONE: the alias's
    /// own body names it back directly (`type Foo3<T> = { y: Foo3<…> }`), as
    /// opposed to reaching itself through other aliases. Also a property of the
    /// alias graph rather than of the entry point — a body that names itself
    /// does so from every entry — so it is order-independent for the same
    /// reason `alias_recursive` is.
    alias_self_recursive: IntMap(SymbolId, void) = .empty,
    /// `aliasGeneric` frames currently materializing, outermost first. Pushed
    /// on entry and popped on exit, so when `aliasInstance` cuts a cycle at an
    /// in-progress alias the suffix from that alias's own frame to the top IS
    /// the cycle the reference just closed. A cycle-detection stack, which the
    /// module rules allow as mutable state; it is bounded by `max_alias_depth`.
    alias_stack: std.ArrayListUnmanaged(SymbolId) = .empty,
    ...
    /// The `reduce_label` nodes whose reductions are in effect on the current
    /// walk, outermost first — tsc's saved/restored `target.antecedents`
    /// (`FlowFlags.ReduceLabel`), which ztsc cannot express as a mutation
    /// because its flow graph is sealed and shared between checkers. Indexed
    /// by `RefKey.reduceDepth`, which is what makes the stack agree with the
    /// memo key; entry `d - 1` is the reduction a depth-`d` query is under.
    /// A cycle-free stack, not a cache: entries past the current depth are
    /// stale and are truncated before the next push.
    flow_reduce: std.ArrayList(FlowId) = .empty,
    ...
    /// `keyof.symbolNamedKeyType` per member ATOM: the key type of a
    /// symbol-named member, or `no_type` for an ordinary name. A pure
    /// function of the name, but not a cheap one — it reads the name, and
    /// `Interner.lookup` takes the interner's shard mutex — while the caller
    /// asks it for every property of every table `keyof` expands. Memo, not
    /// state: the answer never changes for an atom.
    sym_key_cache: IntMap(Atom, TypeId) = .empty,
    ...
    infer_next: u32 = 1,
    /// TS 4.8 constrained `infer`: binder id -> `{ constraint, name }`, for
    /// the binders that carry one. Filled by `inferVarFromNode` as the extends
    /// clause is built and consumed by `conditionalTypeFromNode`, which
    /// expresses the constraint as a conditional wrapped around the true
    /// branch (`V extends C ? True : False`) — the encoding that lets an
    /// ordinary `instantiateId` substitute the constraint, which a side-table
    /// lookup at reduction time could not. A side table because the
    /// constraint is not part of the `infer_var` type's identity.
    infer_constraints: std.AutoHashMapUnmanaged(u32, InferConstraint) = .empty,
    ...
    pub const resolveNamespaceSpace = names_zig.resolveNamespaceSpace;
    ...
    pub const baseRefCut = instantiate_zig.baseRefCut;
    ...
    pub const enumOfMemberSym = enums_zig.enumOfMemberSym;
    pub const enumMemberSymbolType = enums_zig.enumMemberSymbolType;
    ...
    pub const seedStaticFieldContext = enums_zig.seedStaticFieldContext;
    ...
    pub const rebindCondInferVars = generics_zig.rebindCondInferVars;
    ...
    pub const sigListOverlap = assign_zig.sigListOverlap;
    pub const sigOverlap = assign_zig.sigOverlap;
    ...
    pub const excessPropertyFailure = assign_zig.excessPropertyFailure;
    ...
    pub const jsxChildIsSemantic = expr_zig.jsxChildIsSemantic;
    pub const jsxChildCtxAt = expr_zig.jsxChildCtxAt;
    ...
    pub const templateAtom = atoms_zig.templateAtom;
    ...
    pub const hasUndefinedMember = flow_zig.hasUndefinedMember;
    pub const thisPropUnassigned = flow_zig.thisPropUnassigned;
    ...
    pub const isEvolvingArrayVar = flow_zig.isEvolvingArrayVar;
    pub const evolvingArrayStillAuto = flow_zig.evolvingArrayStillAuto;
