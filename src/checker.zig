                ...
                if (c.ts.kind(r) == .tuple) {
                    ...
                    if (v == @floor(v) and v >= 0) {
                        ...
                        if (i < c.ts.tupleLen(r)) {
                            ...
                            // Decisive for cva's `VariantProps` chain: with a
                            // correct `Parameters<C>` tuple whose first element is
                            // optional (`[props?: P]`), `Parameters<C>[0]` must be
                            // `P | undefined` so `OmitUndefined<…>` then strips it —
                            // dropping the `undefined` here left ztsc over-strict and
                            // rejected valid JSX props (TS2322 `IntrinsicAttributes & X`).
