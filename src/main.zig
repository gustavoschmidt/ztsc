...
//!
//! Scope: load files in parallel (mmap + line tables), scan them in
//! parallel (benchmark path), parse them in parallel into sealed
...
pub fn main(init: std.process.Init) !void {
    ...
    // --- Phase: check (single-threaded; partitioned later) ---------------
    ...
    if (cli.memory) {
        ...
        // Checker statistics (bytes/type is the key memory metric).
