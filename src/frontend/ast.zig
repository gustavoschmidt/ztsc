//! Data-oriented AST, modeled on the Zig compiler's own Ast.
//!
//! Design:
//!
...
/// Local alias for the file-offset span type; `Ast.span` returns it.
/// Consumers outside the front end import it from source.zig directly.
...
pub fn wellKnownSymbolKey(name: []const u8) ?[]const u8 {
    ...
    // member. The real vendored lib uses all of these.
...
/// node's `data.lhs` and read back by the census (`--census`) to produce a
/// by-construct frequency table over real code — the table that decides upcoming
...
pub const Tag = enum(u8) {
    ...
    /// data.lhs = an `UnsupportedKind` (the construct, for the census).
    unsupported,
    ...
    qualified_name,
    /// `import("m")` in type position. main_token = `import` keyword;
    ...
    import_type,
    /// `C extends E ? T : F` conditional type. main_token = `extends`
    ...
    mapped_type_node,
    /// `` `head${h0}c0${h1}c1…` `` template-literal type. main_token =
...
/// Payload for a `conditional_type` node: the three type nodes other
...
pub const ForInOf = struct { left: Node, right: Node, is_await: u32 };
...
pub const Ast = struct {
    ...
    /// The construct an `.unsupported` node covers (census). Only valid on
    ...
    /// The key memory metric: (node SoA + extra_data) bytes per node.
    ...
    /// Forwards to ast_dump.zig; kept as a method so callers stay put.
    pub fn dump(a: *const Ast, src: []const u8, w: *std.Io.Writer, node: Node) std.Io.Writer.Error!void {
        return @import("ast_dump.zig").dump(a, src, w, node);
    }
};
