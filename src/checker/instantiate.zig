...
//! its body, `ref(sym, args)` materialized into a structural shape, the LAZY
//! per-member tables, and the nominal `interface` / `class` shapes those
//! references point at.
//!
//! The recursive-alias termination heuristics were split out into
//! `shrink.zig` and are re-exported at the bottom of this file, because
//! `Checker`'s alias block names them here.
