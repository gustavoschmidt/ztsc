//! Memoized type-parameter substitution and fresh-type-parameter minting.
//!
//! One question, asked everywhere in the checker: what does a type become
//! once a set of type parameters is replaced? `instantiate` is the entry
//! point, `instantiateId` the memoized recursive walk, and the structural
//! predicates around them (`containsTypeParam`, `containsFreeTypeParam`,
//! `tpMentions`, `boundMayMove`) are the gates that let most of the walk be
//! skipped outright.
//!
//! A signature that declares its OWN type parameters cannot simply be
//! rewritten in place when the map moves one of their bounds, so this module
//! also owns the fresh-symbol machinery — `mintFreshTp` /
//! `mintFreshTpDeferred` / `resolveFreshBound` — that clones such a parameter
//! with its substituted constraint and default. `substThis` performs the
//! parallel rewrite for the polymorphic `this` marker and shares that symbol
//! pool through `mintThisTp`.
//!
//! Every memo lives on the `Checker` (`inst_cache`, `fresh_tp_ids`,
//! `this_tp_ids`, `tp_mentions`, `ctp_cache`, `ctt_cache`, `subst_this_cache`),
//! so functions take the `Checker` context as their first parameter.
...
/// The already-substituted bounds a fresh type parameter is minted with —
/// the tail of `FreshTp` that is not derived from `orig`. Passed as a record
/// because that is what it becomes.
pub const FreshBounds = struct {
    /// Enforced constraint (`no_type` when the bound rides unenforced).
    constraint: TypeId = types.no_type,
    default: TypeId = types.no_type,
    has_default: bool = false,
    /// Unenforced substituted bound, for the literal-widening rule.
...
/// already-`map`-substituted bounds. Deterministic and memoized per
/// `(orig, canonical map)`, so a repeat instantiation reuses the same id
/// (interning coherence).
pub fn mintFreshTp(c: *Checker, orig: SymbolId, map: []const TpMap, map_id: ?u32, bounds: FreshBounds) Error!u32 {
    ...
    if (!gop.found_existing) {
        ...
        try c.fresh_tp_info.append(c.cm(), .{
            ...
            .has_default = bounds.has_default,
            .const_tp = c.isConstTypeParamSym(orig),
            .orig = c.tpOrigin(orig),
            .widen_bound = bounds.widen_bound,
        });
    }
    return gop.value_ptr.*;
}

/// The bounds a DEFERRED fresh type parameter is minted with: the default is
/// already substituted, the constraint is not (see `mintFreshTpDeferred`).
pub const DeferredBounds = struct {
    /// The UNSUBSTITUTED constraint, forced by `resolveFreshBound`.
    pending_bound: TypeId,
    default: TypeId = types.no_type,
    has_default: bool = false,
    /// Whether the substituted constraint is enforced once forced.
    enforce: bool = false,
    /// Whether the default actually moved — tells a resolution a speculative
    /// mint (`boundMayMove` said "may") from a real one.
...
/// anybody asks for the parameter's constraint. Everything else — the name,
/// the `const` modifier, the origin, and the already-substituted default — is
/// identical to the eager mint, and the record's identity still keys on
/// `(orig, mid)` alone, so deferral does not change which fresh symbol a given
/// (parameter, map) pair gets.
pub fn mintFreshTpDeferred(c: *Checker, orig: SymbolId, mid: u32, bounds: DeferredBounds) Error!u32 {
    const key = (@as(u64, orig) << 32) | mid;
    const gop = try c.fresh_tp_ids.getOrPut(c.cm(), key);
    if (!gop.found_existing) {
        gop.value_ptr.* = c.fresh_tp_next;
        ...
        try c.fresh_tp_info.append(c.cm(), .{
            ...
            .pending_bound = bounds.pending_bound,
            .pending_map = mid,
            .pending_enforce = bounds.enforce,
            .pending_default_moved = bounds.default_moved,
        });
...
fn eagerBound(
    ...
    const fresh = try c.mintFreshTp(tp, cur_map, cur_id, .{
        .constraint = fc,
        .default = nd,
        .has_default = od != types.no_type,
        .widen_bound = wb,
    });
            ...
            for (0..n_tps) |tp_i| {
                ...
                if (rewritable) {
                    ...
                    if (may_move and cur_id != null) {
                        ...
                        fresh = try c.mintFreshTpDeferred(tp, cur_id.?, .{
                            .pending_bound = oc,
                            .default = nd,
                            .has_default = od != types.no_type,
                            .enforce = eligible and c.ts.kind(oc) != .type_param,
                            .default_moved = nd != od,
                        });
                    } else {
