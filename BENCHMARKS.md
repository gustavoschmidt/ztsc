# ZTSC M6 — Benchmark Report

The scoreboard for milestone M6 (ROADMAP.md §4): how a data-oriented,
arena-allocated, parallel checker in Zig compares against `tsc` 5.5.4 and
`tsgo` (the TypeScript 7 native preview) — **on the ztsc M6 subset**
...
Both acceptance gates pass (§5): wall well within 1.25× of tsgo
...
All code stays inside the ztsc M6 subset (ROADMAP.md §6) and is lib-free
...
The M6 gate: *on medium + large corpora, wall clock within 1.25× of tsgo
...
- **ztsc checks a subset** (ROADMAP.md §6). No conditional/mapped/
