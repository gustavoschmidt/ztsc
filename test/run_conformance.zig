//! Conformance suite runner (single-file + multi-file).
//!
...
//! - **Directory**: a folder containing `entry.ts`; the whole module
...
//!
//! This file also hosts the multi-file determinism and cycle-stress tests:
...
fn renderProgramDiags(
    ...
    // One shared frozen base across all overlay checkers: this is the
...
test "determinism: cross-file base cycles report identically for N = 1, 2, 4, 8" {
    ...
    const br = try modules.buildProgram(alloc, io, gpa, &interner, d, &.{"entry.d.ts"}, .none, .{}, .{}, null);
