...
//! against tsc.
//!
//! there are no cases and no checker yet, so discovery must find zero
...
test "conformance: discover and run cases" {
    ...
    for (cases.items) |case| {
        // no checker yet. Once the checker lands, run it here and diff
        // diagnostics against the .expected snapshot.
        std.debug.print("conformance: case {s} skipped (no checker yet)\n", .{case.name});
        failed += 1;
    }

    // Zero cases at bootstrap; any case added before the checker exists must fail
