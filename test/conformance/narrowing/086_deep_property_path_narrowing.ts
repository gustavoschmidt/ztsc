// A reference path deeper than `max_ref_depth` does not fit inline in a
// `RefKey`; it is interned instead (`makeRefKey`) so paths up to
// `max_deep_ref_depth` still narrow. Five links is what a `this.state.a.b.c`
// guard needs.
...
// A destructuring binding inherits the narrowing of `<init>.key`, which pushes
// an already-full inline path over the edge (`extendRefKey`).
export const j = () => {
  if (o.a.b.c.d) {
    const { d } = o.a.b.c;
    f(d);
